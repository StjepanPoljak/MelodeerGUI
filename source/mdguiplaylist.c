...
void MDGUIPB__update(MDGUI__playlist_box_t *playlistbox) {
	...
	if (MDGUI__str_array_is_empty(&playlistbox->filenames)) {
		...
		MDGUI__str_array_copy_raw(&playlistbox->filenames,
					  &playlistbox->listbox.str_array,
					  0, playlistbox->filenames.cnum - 1,
					  NULL, MDGUI__str_transform_fname);
	}
