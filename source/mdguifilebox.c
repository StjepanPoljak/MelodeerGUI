...
// 1 = yes, 0 = is dir, -1 = is_empty
int MDGUIFB__is_selected_a_file(MDGUI__file_box_t *filebox) {

	if (filebox->listbox.str_array.cnum < 1)
		return -1;

	return filebox->listbox.str_array.carray[filebox->listbox.num_selected][0] == 'f';
}
...
void MDGUIFB__stack_push(MDGUI__file_box_t *filebox) {
	...
	char *filename = filebox->listbox.str_array
			.carray[filebox->listbox.num_selected];

	MDGUIFB__stack_element_init(
			new_stack_el, &(filename[1]),
			filebox->listbox.num_first,
			filebox->listbox.num_selected
			);
...
void MDGUIFB__stack_pop(MDGUI__file_box_t *filebox) {
	...
	if (filebox->last->previous) {
		filebox->last = filebox->last->previous;
	}
	else {
		filebox->last = NULL;
	}
...
bool MDGUIFB__return(MDGUI__file_box_t *filebox) {
	...
	if (filebox->listbox.num_selected < 0 ||
	    filebox->listbox.num_selected >= filebox->listbox.str_array.cnum) {
		return false;
	}

	char *selected = filebox->listbox.str_array
			.carray[filebox->listbox.num_selected];
	int selected_size = 0;
	...
	case 'd':
		selected_size = MDGUI__get_string_size (&(selected[1]));

		if ((selected_size == 1 && selected[1] == '.') ||
		    selected_size == 0) {
			return false;
		}
...
bool MDGUI__compare(char *string1, char *string2) {
	...
	int min_size = string1_size > string2_size
		     ? string2_size : string1_size;
...
void MDGUIFB__serialize_curr_dir(MDGUI__file_box_t *filebox, char **curr_dir) {
	...
	if (!filebox->last) {
		...
		if (*curr_dir) {
			*curr_dir = realloc(*curr_dir, sizeof(**curr_dir) * 2);
		}
		else {
			*curr_dir = malloc (sizeof (**curr_dir) * 2);
		}
	...
	*curr_dir = *curr_dir
		  ? realloc(*curr_dir, sizeof(**curr_dir) * curr_dir_size)
		  : malloc(sizeof(**curr_dir) * curr_dir_size);
